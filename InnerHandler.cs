...
public class InnerHandler
{
    ...
    public async Task<APIGatewayProxyResponse> Handle(SocketMessage<JoinX01QueueCommand> request)
    {
        // Check if the request or its message is null
        if (request?.Message is null)
            throw new BadRequestException("Unable to parse request.", typeof(JoinX01QueueCommand));
